...
my %skip1 = map { ($_, 1) } qw (DR_HOOK ABOR1 ABORT INIT FINAL UPDATE_VIEW SYNC_HOST_RDONLY SYNC_HOST_RDWR SAVE COPY WIPE
                                DIFF INITOPTIONS GETOPTION LOAD CHECKOPTIONS UPDATE SYNC_DEVICE_RDONLY SYNC_DEVICE_RDWR);
