...
IF (YDXFU%YTRDU%LACTIVE) CALL COPY3 (YDXFU%YTRDU, YDXFU%TRDU, PSTRDU)
IF (YDXFU%YTRDV%LACTIVE) CALL COPY3 (YDXFU%YTRDV, YDXFU%TRDV, PSTRDV)
IF (YDXFU%YTRCU%LACTIVE) CALL COPY3 (YDXFU%YTRCU, YDXFU%TRCU, PSTRCU)
IF (YDXFU%YTRCV%LACTIVE) CALL COPY3 (YDXFU%YTRCV, YDXFU%TRCV, PSTRCV)
IF (YDXFU%YDICQ%LACTIVE) CALL COPY3 (YDXFU%YDICQ, YDXFU%DICQ, YDMF_PHYS_OUT%DIFCQ)
IF (YDXFU%YDICS%LACTIVE) CALL COPY3 (YDXFU%YDICS, YDXFU%DICS, YDMF_PHYS_OUT%DIFCS)
IF (YDXFU%YTRTU%LACTIVE) CALL COPY3 (YDXFU%YTRTU, YDXFU%TRTU, PSTRTU)
IF (YDXFU%YTRTV%LACTIVE) CALL COPY3 (YDXFU%YTRTV, YDXFU%TRTV, PSTRTV)
IF (YDXFU%YDITQ%LACTIVE) CALL COPY3 (YDXFU%YDITQ, YDXFU%DITQ, YDMF_PHYS_OUT%DIFTQ)
IF (YDXFU%YDITS%LACTIVE) CALL COPY3 (YDXFU%YDITS, YDXFU%DITS, YDMF_PHYS_OUT%DIFTS)
IF (YDXFU%YPLCL%LACTIVE) CALL COPY3 (YDXFU%YPLCL, YDXFU%PLCL, YDMF_PHYS_OUT%FPLCL)
IF (YDXFU%YPLCN%LACTIVE) CALL COPY3 (YDXFU%YPLCN, YDXFU%PLCN, YDMF_PHYS_OUT%FPLCN)
IF (YDXFU%YPLCG%LACTIVE) CALL COPY3 (YDXFU%YPLCG, YDXFU%PLCG, YDMF_PHYS_OUT%FPLCG)
IF (YDXFU%YPLCH%LACTIVE) CALL COPY3 (YDXFU%YPLCH, YDXFU%PLCH, YDMF_PHYS_OUT%FPLCH)
IF (YDXFU%YPLSL%LACTIVE) CALL COPY3 (YDXFU%YPLSL, YDXFU%PLSL, YDMF_PHYS_OUT%FPLSL)
IF (YDXFU%YPLSN%LACTIVE) CALL COPY3 (YDXFU%YPLSN, YDXFU%PLSN, YDMF_PHYS_OUT%FPLSN)
IF (YDXFU%YPLSG%LACTIVE) CALL COPY3 (YDXFU%YPLSG, YDXFU%PLSG, YDMF_PHYS_OUT%FPLSG)
IF (YDXFU%YPLSH%LACTIVE) CALL COPY3 (YDXFU%YPLSH, YDXFU%PLSH, YDMF_PHYS_OUT%FPLSH)
IF (YDXFU%YRSO %LACTIVE) CALL COPY3 (YDXFU%YRSO , YDXFU%RSO , YDMF_PHYS_OUT%FRSO(:,:,1))
IF (YDXFU%YRTH %LACTIVE) CALL COPY3 (YDXFU%YRTH , YDXFU%RTH , YDMF_PHYS_OUT%FRTH(:,:,1))
IF (YDXFU%YNEB %LACTIVE) CALL COPY3 (YDXFU%YNEB , YDXFU%NEB , YDCPG_MISC%NEB)
IF (YDXFU%YQICE%LACTIVE) CALL COPY3 (YDXFU%YQICE, YDXFU%QICE, YDCPG_MISC%QICE)
IF (YDXFU%YQLI %LACTIVE) CALL COPY3 (YDXFU%YQLI , YDXFU%QLI , YDCPG_MISC%QLI)

IF (YDXFU%YNEBT %LACTIVE) CALL COPY1 (YDXFU%NEBT , YDCPG_MISC%CLCT     )
IF (YDXFU%YFEVL %LACTIVE) CALL COPY1 (YDXFU%FEVL , YDMF_PHYS_OUT%FEVL(:,1))
IF (YDXFU%YFONTE%LACTIVE) CALL COPY1 (YDXFU%FONTE, YDMF_PHYS_OUT%FONTE )
IF (YDXFU%YFCHSP%LACTIVE) CALL COPY1 (YDXFU%FCHSP, YDMF_PHYS_OUT%FCHSP(:,1))
IF (YDXFU%YFLWSP%LACTIVE) CALL COPY1 (YDXFU%FLWSP, YDMF_PHYS_OUT%FLWSP )
IF (YDXFU%YRUISS%LACTIVE) CALL COPY1 (YDXFU%RUISS, YDMF_PHYS_OUT%RUISS )
IF (YDXFU%YRUISP%LACTIVE) CALL COPY1 (YDXFU%RUISP, YDMF_PHYS_OUT%RUISP )
IF (YDXFU%YFEVV %LACTIVE) CALL COPY1 (YDXFU%FEVV , YDMF_PHYS_OUT%FEVV  )
IF (YDXFU%YFTR  %LACTIVE) CALL COPY1 (YDXFU%FTR  , YDMF_PHYS_OUT%FTR   )
IF (YDXFU%YRUISL%LACTIVE) CALL COPY1 (YDXFU%RUISL, YDMF_PHYS_OUT%RUISL )
IF (YDXFU%YNBCON%LACTIVE) CALL COPY1 (YDXFU%NBCON, YDMF_PHYS_OUT%CLCC  )
IF (YDXFU%YNBHAU%LACTIVE) CALL COPY1 (YDXFU%NBHAU, YDMF_PHYS_OUT%CLCH  )
IF (YDXFU%YNBMOY%LACTIVE) CALL COPY1 (YDXFU%NBMOY, YDMF_PHYS_OUT%CLCM  )
IF (YDXFU%YNBBAS%LACTIVE) CALL COPY1 (YDXFU%NBBAS, YDMF_PHYS_OUT%CLCL  )
IF (YDXFU%YCAPE %LACTIVE) CALL COPY1 (YDXFU%CAPE , YDMF_PHYS_OUT%CAPE  )
IF (YDXFU%YCTOP %LACTIVE) CALL COPY1 (YDXFU%CTOP , YDMF_PHYS_OUT%CTOP  )
IF (YDXFU%YMOCON%LACTIVE) CALL COPY1 (YDXFU%MOCON, YDMF_PHYS_OUT%MOCON )
IF (YDXFU%YCLPH %LACTIVE) CALL COPY1 (YDXFU%CLPH , YDMF_PHYS_OUT%CLPH  )
IF (YDXFU%YVEIN %LACTIVE) CALL COPY1 (YDXFU%VEIN , YDMF_PHYS_OUT%VEIN  )
IF (YDXFU%YMRT  %LACTIVE) CALL COPY1 (YDXFU%MRT  , YDMF_PHYS_OUT%MRT   )
IF (YDXFU%YTCLS %LACTIVE) CALL COPY1 (YDXFU%TCLS , YDMF_PHYS_OUT%TCLS  )
IF (YDXFU%YSIC  %LACTIVE) CALL COPY1 (YDXFU%SIC  , YDMF_PHYS_OUT%SIC   )
IF (YDXFU%YQCLS %LACTIVE) CALL COPY1 (YDXFU%QCLS , YDMF_PHYS_OUT%QCLS  )
IF (YDXFU%YUGST %LACTIVE) CALL COPY1 (YDXFU%UGST , PXUGST, LDMASK=LXXGST, LDVMASK=LLGST)
IF (YDXFU%YVGST %LACTIVE) CALL COPY1 (YDXFU%VGST , PXVGST, LDMASK=LXXGST, LDVMASK=LLGST)
IF (YDXFU%YUGST2%LACTIVE .AND. LXXGST2) CALL COPY1 (YDXFU%UGST2, PXUGST, LDMASK=.TRUE., LDVMASK=LLGST2)
IF (YDXFU%YVGST2%LACTIVE .AND. LXXGST2) CALL COPY1 (YDXFU%VGST2, PXVGST, LDMASK=.TRUE., LDVMASK=LLGST2)
IF (YDXFU%YUCLS %LACTIVE) CALL COPY1 (YDXFU%UCLS , ZUMEAN , PXUCLS , LDCOND=LXMWINDCLS)
IF (YDXFU%YVCLS %LACTIVE) CALL COPY1 (YDXFU%VCLS , ZVMEAN , PXVCLS , LDCOND=LXMWINDCLS)
IF (YDXFU%YNUCLS%LACTIVE) CALL COPY1 (YDXFU%NUCLS, ZNUMEAN, PXNUCLS, LDCOND=LXMWINDCLS)
IF (YDXFU%YNVCLS%LACTIVE) CALL COPY1 (YDXFU%NVCLS, ZNVMEAN, PXNVCLS, LDCOND=LXMWINDCLS)

IF (YDXFU%YXDIAGH%LACTIVE) THEN
  CALL COPY1 (YDXFU%XDIAGH, PVMAX=YDMF_PHYS_OUT%DIAGH, LDRESET=LRESET_GST, PRESET=0.0_JPRB)
ENDIF

IF (YDXFU%YVISICLD%LACTIVE) THEN
  CALL COPY1 (YDXFU%VISICLD, PVMIN=YDMF_PHYS_OUT%VISICLD, PSMAX=0.0_JPRB, &
            & LDRESET=LRESET_VISI, PRESET=20000.0_JPRB)
ENDIF

IF (YDXFU%YVISIHYD%LACTIVE) THEN
  CALL COPY1 (YDXFU%VISIHYD, PVMIN=YDMF_PHYS_OUT%VISIHYD, PSMAX=0.0_JPRB, &
            & LDRESET=LRESET_VISI, PRESET=20000.0_JPRB)
ENDIF

IF (YDXFU%YCLWC%LACTIVE) THEN
  CALL COPY1 (YDXFU%CLWC, PVMAX=YDMF_PHYS_OUT%MXCLWC, LDRESET=LRESET_VISI, PRESET=0.0_JPRB)
...
IF (YDXFU%YVISICLD2%LACTIVE) THEN
  CALL COPY1 (YDXFU%VISICLD2, PVMIN=YDMF_PHYS_OUT%VISICLD, PSMAX=0.0_JPRB, &
            & LDRESET=LRESET_VISI2, PRESET=20000.0_JPRB)
ENDIF

IF (YDXFU%YVISIHYD2%LACTIVE) THEN
  CALL COPY1 (YDXFU%VISIHYD2, PVMIN=YDMF_PHYS_OUT%VISIHYD, PSMAX=0.0_JPRB, &
            & LDRESET=LRESET_VISI2, PRESET=20000.0_JPRB)
ENDIF

IF (YDXFU%YCLWC2%LACTIVE) THEN
  CALL COPY1 (YDXFU%CLWC2, PVMAX=YDMF_PHYS_OUT%MXCLWC, LDRESET=LRESET_VISI2, PRESET=0.0_JPRB)
  YDXFU%CLWC2(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=MAX(YDXFU%CLWC2(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA),0.0_JPRB)
ENDIF

IF (YDXFU%YTPWCLS%LACTIVE) CALL COPY1 (YDXFU%TPWCLS, YDMF_PHYS_OUT%TPWCLS)

IF (YDXFU%YRHCLS%LACTIVE) THEN
  CALL COPY1 (YDXFU%RHCLS, YDMF_PHYS_OUT%RHCLS)
  YDXFU%RHCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=MIN(1.0_JPRB,MAX(0.0_JPRB,YDXFU%RHCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)))
ENDIF

IF (YDXFU%YTN%LACTIVE) CALL COPY1 (YDXFU%TN, PVMIN=YDMF_PHYS_OUT%TCLS, LDRESET=LRESET, PRESET=100000._JPRB)
IF (YDXFU%YTX%LACTIVE) CALL COPY1 (YDXFU%TX, PVMAX=YDMF_PHYS_OUT%TCLS, LDRESET=LRESET, PRESET=0.0_JPRB)

IF (YDXFU%YHUN%LACTIVE) THEN
  CALL COPY1 (YDXFU%HUN, PVMIN=YDMF_PHYS_OUT%RHCLS, LDRESET=LRESET, PRESET=1.0_JPRB)
...
IF (YDXFU%YHUX%LACTIVE) THEN
  CALL COPY1 (YDXFU%HUX, PVMAX=YDMF_PHYS_OUT%RHCLS, LDRESET=LRESET, PRESET=0.0_JPRB)
  YDXFU%HUX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=MAX(0.0_JPRB,MIN(1.0_JPRB,YDXFU%HUX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)))
ENDIF

IF (YDXFU%YTHW%LACTIVE) CALL COPY1 (YDXFU%THW, YDMF_PHYS_OUT%FCS (:,1))
...
SUBROUTINE COPY1 (PXFU, PPHY0, PPHY1, LDCOND, LDMASK, LDVMASK, &
