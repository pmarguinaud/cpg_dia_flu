...
CHARACTER*32 :: CLPROC
CHARACTER*256 :: CLFILE, CLCASE, CLOUT
INTEGER (KIND=JPIM) :: JLON, IBL, NGPBLKS, NPROMA
INTEGER (KIND=JPIM) :: NTIME, ITIME
INTEGER (KIND=JPIM) :: NSTACKSIZE
LOGICAL :: LLDIFF, LLSYNCHOST, LLPARALLELVIEW, LLSINGLECOLUMNFIELDAPI 
LOGICAL :: LLPARALLELFIELDAPI, LLPARALLELSINGLECOLUMNFIELDAPI
...
CALL GETOPTION ("--sync-host", LLSYNCHOST)
CALL GETOPTION ("--parallel-view", LLPARALLELVIEW)
CALL GETOPTION ("--parallel-field-api", LLPARALLELFIELDAPI)
CALL GETOPTION ("--parallel-single-column-field-api", LLPARALLELSINGLECOLUMNFIELDAPI)
CALL GETOPTION ("--single-column-field-api", LLSINGLECOLUMNFIELDAPI)
...
DO ITIME = 1, NTIME
  ...
  IF (LLPARALLELSINGLECOLUMNFIELDAPI) THEN
    CALL CPG_DIA_FLU_PARALLEL_SINGLE_COLUMN_FIELD_API &
...
!$OMP DO SCHEDULE(DYNAMIC,1)
  ...
  DO IBL = 1, NGPBLKS
    ...
    IF (LLSINGLECOLUMNFIELDAPI) THEN
      CALL YLCPG_BNDS%UPDATE (IBL)
      YLSTACK%L = LOC (YLSTACK%F_P%PTR (1,IBL))
      YLSTACK%U = YLSTACK%L + KIND (YLSTACK%F_P%PTR) * SIZE (YLSTACK%F_P%PTR (:,IBL))
...
IF (LLDIFF) THEN
  CLFILE = TRIM (CLCASE)//"/YDXFU.OUT"//TRIM (CLPROC)
  OPEN (77, FILE=TRIM (CLFILE), FORM='UNFORMATTED', STATUS='OLD')
  ...
  CLFILE = TRIM (CLCASE)//"/YDCFU.OUT"//TRIM (CLPROC)
  OPEN (77, FILE=TRIM (CLFILE), FORM='UNFORMATTED', STATUS='OLD')
