...
CLASS (TSRFTLAD), INTENT (IN), TARGET :: YD
...
IMPLICIT NONE
INTEGER, INTENT (IN) :: KLUN
CLASS (TSRFTLAD), INTENT (OUT), TARGET :: YD
INTEGER :: IL3(3), IU3(3)
LOGICAL :: LGPTSKIN0, LGPTSKIN9
...
SUBROUTINE WIPE_TSRFTLAD (YD, LDDELETED)
IMPLICIT NONE
CLASS (TSRFTLAD), INTENT (IN), TARGET :: YD
...
IF (LGPTSKIN0) THEN
  ...
  !$acc exit data delete (YD%GPTSKIN0)
ENDIF

LGPTSKIN9 = ALLOCATED (YD%GPTSKIN9)
IF (LGPTSKIN9) THEN
  !$acc exit data detach (YD%GPTSKIN9)
  !$acc exit data delete (YD%GPTSKIN9)
ENDIF
